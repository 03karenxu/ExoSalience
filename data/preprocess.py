...
import third_party.kepler_spline
...
def process_light_curve(all_time, all_flux):
  ...
  spline = third_party.kepler_spline.fit_kepler_spline(all_time, all_flux, verbose=False)[0]
...
def generate_dict_for_tce(time, flux, tce):
    """Generates a dict representing an input TCE.
    ...
    Returns:
        A dict containing global view, local view, label, id, tabular data
    """
    period = tce["tce_period"]
    duration = tce["tce_duration"]
    t0 = tce["tce_time0bk"]
    label = tce["av_training_set"]
    kepid = tce["kepid"]
    ...
    # iterate over all columns in the TCE record
    tabular_values = []
    for name, value in tce.items():
        if name not in EXCLUDED_KEYS:
            # attempt to cast the value to float32 before adding to the tabular list
            try:
                tabular_values.append(float(value))
            except ValueError: # skips non numerical categories***
                continue
          
    # init global view and local view
    output_dict = {
        "global_view": global_view(time, flux, period),
        "local_view": local_view(time, flux, period, duration),
        "label": label,
        "kepid": np.int32(kepid),
        "tabular": np.array(tabular_values, dtype=float)
    }
